...
//
// Toast Viewport (Position Control)
//
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ...
    className={cn(
      ...
      "fixed z-[50] flex w-full flex-col-reverse p-4 pointer-events-none",
      "top-0 left-1/2 -translate-x-1/2 sm:translate-x-0 sm:left-auto sm:right-0 sm:flex-col md:max-w-[420px]",
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
    ...
    "data-[state=open]:slide-in-from-bottom-full sm:data-[state=open]:slide-in-from-right-full",
  {
    variants: {
      variant: {
        // ✅ White background for normal toasts
        default:
          "border border-gray-200 bg-white text-gray-900 shadow-lg",
        // ✅ Red background for destructive (error) toasts
        destructive:
          "border-red-300 bg-red-50 text-red-700 group destructive shadow-lg",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute outline-none right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity " +
        "group-hover:opacity-50 group-[.destructive]:text-red-700 hover:text-foreground " +
        " focus:opacity-100 focus:outline-none " +
        "focus:ring-2 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600",
      className
    )}
    toast-close=""
    {...props}
